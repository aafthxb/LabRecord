...
function handleBack(event) {
    ...
    if (App.packages.openPackage) {
        runWithTactileDelay(event, () => {
            ...
            const folder = App.packages.openPackage;
            collapseAllPackageFileCards(folder);
            document.getElementById(`package-${folder}-container`)
                ?.classList.remove("active");

            App.packages.openPackage = null;
            App.packages.onPackagesList = true;
            ...
            document.getElementById("page-title").textContent = "CUSTOM PACKAGES";
            document.getElementById("page-subtitle").textContent =
                "Multi-file Java packages — browse, search, and check them together.";
...
const App = {
    ...
    // generated/packages-code/<folder>.json). Deliberately separate
    // from the reorder/edit/commit system above — there's no in-browser
    // editor for packages yet, they're added by dropping a folder under
    // packages/ (with a meta.json) and running the generator.
    packages: {
        list: [],
        byFolder: new Map(),
        cards: [],
        fileCards: new Map(),
        builtList: false,
        builtPackages: new Set(),
        codeIndexLoaded: new Set(),
        codeLookup: {},
        onPackagesList: false,
        openPackage: null
    }
};
// ==========================
...
function buildLanguageUI() {
    ...
    // ---------- Packages home tile ----------
    const packagesCard = document.createElement("div");
    packagesCard.className = "folder-card card-enter";
    packagesCard.style.setProperty("--stagger-index", App.languageIndex.length);

    packagesCard.addEventListener(
        "animationend",
        () => packagesCard.classList.remove("card-enter"),
        { once: true }
    );

    packagesCard.onclick = (event) => openPackagesList(event);

    packagesCard.innerHTML = `
        <h2 class="folder-card-title">CUSTOM PACKAGES</h2>
        <p class="folder-card-desc">
            Multi-file Java packages — browse, search, and check them together.
        </p>
    `;

    homeView.appendChild(packagesCard);

    const packagesListContainer = document.createElement("div");
    packagesListContainer.id = "packages-list-container";
    packagesListContainer.className = "view-container";
    document.getElementById("packages-views").appendChild(packagesListContainer);
}
...
async function loadPackagesIndex() {
    try {
        ...
        App.packages.byFolder = new Map(
            App.packages.list.map(pkg => [pkg.folder, pkg])
        );
    } catch {
        App.packages.list = [];
        App.packages.byFolder = new Map();
...
function collapseAllPackageCards() {
    (App.packages.cards || []).forEach(card => {
...
function collapseAllPackageFileCards(folder) {
    (App.packages.fileCards.get(folder) || []).forEach(card => {
        card._collapse?.();
    });
}

function openPackagesList(event) {
    runWithTactileDelay(event, () => {
        ...
        if (!App.packages.builtList) {
            buildPackagesListUI(listContainer);
            App.packages.builtList = true;
        }
...
function buildPackagesListUI(container) {
    container.innerHTML = "";
    ...
    App.packages.list.forEach((pkg, index) => {
        ...
        container.appendChild(card);
        App.packages.cards.push(card);
    });

    setupPackagesListSearch(search);
}
...
function setupPackagesListSearch(search) {
    ...
    function filterCards() {
        ...
        App.packages.cards.forEach(card => { card.style.order = ""; });
        ...
        const total = App.packages.cards.length;
...
async function loadPackageCodeIndex(folder) {
    if (App.packages.codeIndexLoaded.has(folder)) return;

    const response = await fetch(`/generated/packages-code/${encodeURIComponent(folder)}.json`);
    if (!response.ok) {
    ...
    App.packages.codeIndexLoaded.add(folder);
}
...
function setupPackageFileSearch(search, pkg, cardsList) {
    ...
    async function filterCards() {
        ...
        if (query && metadataMatches === 0) {
            ...
            try {
                await loadPackageCodeIndex(pkg.folder);
            } catch {
...
function buildPackageFileView(container, pkg) {
    ...
    const cardsList = [];
    App.packages.fileCards.set(pkg.folder, cardsList);
...
function openPackage(folder, event) {
    runWithTactileDelay(event, () => {

        const pkg = App.packages.byFolder.get(folder);
        if (!pkg) return;

        collapseAllPackageCards();

        App.packages.onPackagesList = false;
        App.packages.openPackage = folder;
        ...
        let container = document.getElementById(`package-${folder}-container`);

        if (!container) {
            container = document.createElement("div");
            container.id = `package-${folder}-container`;
            container.className = "view-container";
            document.getElementById("packages-views").appendChild(container);
        }

        if (!App.packages.builtPackages.has(folder)) {
            buildPackageFileView(container, pkg);
            App.packages.builtPackages.add(folder);
        } else {
            collapseAllPackageFileCards(folder);
        }
