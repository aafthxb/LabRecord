...
// ==========================
//
// A "package" is a folder under ./packages/<PackageFolder>/ containing
// a meta.json ({ name, description, language? }) plus its source
// files. Unlike programs/, packages are not reordered or edited
// in-browser — this generator is the only thing that produces their
// index, so a package's list order is just alphabetical-by-folder,
// same as everything else discovered from disk.
//
// Two outputs:
//  - generated/packages-index.json: everything the packages-list page
//    and each package's file-cards page need (name, description, file
//    metadata, and a rolled-up `searchText` for the list page's
//    fast/metadata-only search — see chat notes: the top-level list
//    intentionally never touches code content).
//  - generated/packages-code/<folder>.json: one file PER package with
//    that package's file contents, fetched lazily only once someone
//    opens that specific package and its fast-tier search comes up
//    empty (mirrors the existing single-folder code-index fallback,
//    just scoped per package instead of one global file).
...
const discoveredPackageFolders = discoverPackages();
const packagesIndex = [];
let totalPackageFiles = 0;

fs.mkdirSync(packagesCodeDir, { recursive: true });

// Clean out per-package code files for packages that no longer exist,
// so a renamed/removed package doesn't leave a stale, unreferenced
// code-fallback file behind.
if (fs.existsSync(packagesCodeDir)) {
  const validNames = new Set(discoveredPackageFolders.map(f => `${f}.json`));
  for (const existing of fs.readdirSync(packagesCodeDir)) {
    if (!validNames.has(existing)) {
      fs.unlinkSync(path.join(packagesCodeDir, existing));
      console.log(`✓ Removed stale packages-code/${existing}`);
    }
  }
}

for (const folder of discoveredPackageFolders) {
  const pkgPath = path.join("./packages", folder);
  ...
  try {
    ...
    console.warn(`⚠ packages/${folder}/meta.json is invalid JSON. Skipping package.`);
    continue;
  }

  const languageId = (meta.language || "java").toLowerCase();
  const language = languages[languageId] || languages.java;

  const files = fs
    .readdirSync(pkgPath)
    .filter(file => language.extensions.includes(path.extname(file).toLowerCase()))
    .sort((a, b) => a.localeCompare(b));

  if (files.length === 0) {
    console.warn(`⚠ packages/${folder} has no ${languageId} source files. Skipping package.`);
    continue;
  }

  const fileEntries = [];
  const codeEntries = [];
  const rollup = [meta.name || folder, meta.description || ""];
  ...
  packagesIndex.push({
    ...
    // class named "Greeter") still surfaces the package on the
    // top-level list — see chat notes on this exact case.
    searchText: normalizeSearchText(rollup.join(" ")),
    files: fileEntries
  });

  writeJsonIfChanged(
    path.join(packagesCodeDir, `${folder}.json`),
    codeEntries
  );
...
console.log(`Packages: ${packagesIndex.length} (${totalPackageFiles} files)`);
console.log("==================================");
